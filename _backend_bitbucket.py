...
class BitbucketBackend(VcsBackend):
    ...
    def setup(self, connection_input: ConnectionInput) -> None:
        ...
        try:
            self.client = Bitbucket(url=connection_input["base_url"], token=connection_input["token"], cloud=False)
            project_gen = self.client.project_list(limit=1)
            first_project = next(project_gen, None)
            if first_project is None:
                raise Exception("No projects found. Check your token and permissions.")
        except Exception as e:
            raise Exception("Failed to setup Bitbucket connection. Check your token and URL.") from e
        return self
    ...
    def fetchAllRepos(self, progress: bool = False, verbose: bool = False) -> Dict[int, Repo]:
        ...
        if self.client is None:
            raise Exception("Bitbucket client is not set up. Call setup() first.")
        try:
            projects = []
        ...
        repos: Dict[int, Repo] = {}
        bar = Bar('Fetching repo data for projects', max=len(projects)) if progress else None

        for project in projects:
            if (project["type"] == "NORMAL"):
                project_key = project.get("key")
                if verbose: print(f"Fetching repo data for project [{project_key}]")
                try:
                    project_repos = []
                    for repo_item in self.client.repo_list(project_key, limit=100):
                        project_repos.append(repo_item)
                except Exception as e:
                    raise Exception(f"Failed to fetch repositories for project {project_key}.") from e
    ...
    def enrichRepo(self, repo: Repo, verbose: bool = False) -> Repo:
        ...
        try:
            ...
            latest_branch = None
            default_branch = None
            for branch in client.get_branches(project_key=repo["group_key"], repository_slug=repo["repo_key"], limit=100):
                if "latestCommit" in branch and branch['latestCommit']:
                    commit_date = datetime.fromtimestamp(branch['metadata']['com.atlassian.bitbucket.server.bitbucket-branch:latest-commit-metadata']['authorTimestamp']/1000, UTC)
                    if latest_commit_date is None or commit_date > latest_commit_date:
                        ...
                        if "displayName" in branch['metadata']['com.atlassian.bitbucket.server.bitbucket-branch:latest-commit-metadata']['author']:
                            contact_name = branch['metadata']['com.atlassian.bitbucket.server.bitbucket-branch:latest-commit-metadata']['author']['displayName']
                        if "emailAddress" in branch['metadata']['com.atlassian.bitbucket.server.bitbucket-branch:latest-commit-metadata']['author']:
